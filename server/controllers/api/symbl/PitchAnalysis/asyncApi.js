...
import { createUserHistoryForAnalysisData } from '../../../cassandra.js';
import { getUserHistoryForAnalysisData } from '../../../cassandra.js';
                    ...
                    generateAuthToken((authToken) => {
                        ...
                        startPitchAnalysis(authToken.accessToken, path, req.body.meetingName, async (err, data) => {
                            if (err) {
                                ...
                                try {
                                    const existingUser = await getUserHistoryForAnalysisData(req.userId)
                                    ...
                                    if (Object.keys(existingUser.data).length == 0) {
                                        try {
                                            ...
                                            await createUserHistoryForAnalysisData({ handlerId: req.userId, conversationIdData: { conversationId: data.messages.messages[0].conversationId, createdAt: Date.now(), meetingName: req.body.meetingName, analysisData: data, url: url } });
                ...
                generateAuthToken((authToken) => {
                    ...
                    startPitchAnalysis(authToken.accessToken, path, req.body.meetingName, async (err, data) => {
                        if (err) {
                            ...
                            try {
                                let existingUser = await getUserHistoryForAnalysisData(req.userId)
                                existingUser = JSON.parse(existingUser)
                                if (Object.keys(existingUser.data).length == 0) {
                                    try {
